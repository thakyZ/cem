...
import net.dorianpb.cem.forge.internal.util.CemStringParser.ParsedFunction.ParsedFunctionBool;
import net.dorianpb.cem.forge.internal.util.CemStringParser.ParsedFunction.ParsedFunctionFloat;
import net.dorianpb.cem.forge.internal.util.CemStringParser.ParsedFunction.ParsedFunctionType;
	...
	enum FLOAT_FUNCTION implements ParsedFunctionFloat {
		...
		public Float eval(ArrayList<ParsedExpression> args, Environment env) {
			return switch (this) {
				...
				case ATAN2 ->
						(float) MathHelper.atan2(((ParsedExpressionFloat) args.get(0)).eval(env), ((ParsedExpressionFloat) args.get(1)).eval(env));
				...
				case POW ->
						(float) Math.pow(((ParsedExpressionFloat) args.get(0)).eval(env), ((ParsedExpressionFloat) args.get(1)).eval(env));
				...
				case FMOD ->
						MathHelper.floorMod(((ParsedExpressionFloat) args.get(0)).eval(env), ((ParsedExpressionFloat) args.get(1)).eval(env));
